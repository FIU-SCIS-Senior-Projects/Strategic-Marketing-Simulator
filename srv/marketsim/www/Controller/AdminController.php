...
class AdminController 
{
	...
	function addAdminPage()
	{
			...
			"<h3>Email</h3>".
			"<input type = 'email' name = 'textbox_email' pattern='[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$' placeholder='admin@marketsim.edu' title='Email and login for the User'>".
			"<h3>Password</h3>".
			...
			"<h3>Secret Answer</h3>".
			"<input type = 'Text' name = 'textbox_secretAnswer'  pattern='.{1,}' placeholder='marketsim-dev.cis.fiu.edu' title='Used for recovering your account'>".
			"<br />".
			"<br />"."<br />".					
	...
	function addBotHotelPage()
	{
		...
		$hotelTypes = "<input type='radio' name='hotelType' value ='economy' id='eco' title='Is the Hotel cheap?'> Economic".
					"<label class='radio-inline' for 'eco'>".
					"</label>".
					"<input type='radio' name='hotelType' value ='midrange' id='mid' title='Is the Hotel between fancy and cheap?'> Midrange".
					"<label class='radio-inline' for 'mid'>".
					"</label>".	
					"<input type='radio' name='hotelType' value ='luxury' id='lux' Title='Is the Hotel Fancy?'> Luxury".	 						
		...
		{
			$locations .="<input type='radio' name='hotelLocation' value =".$temp['id']." id=".$temp['id']." title='Hotels Location'> ".$temp['type'].
		...
		$result = 	"<table class='table'>".
					...
					"<h3>Name of Hotel</h3>".
					"<input type = 'Text' name = 'textbox_Hotel_name' pattern='{5,}' placeholder='FIU inn' title='The Name of the New Hotel for the Bot user'>".

					"<h3>Which Game to Assign bot to?</h3>".
					"<input type = 'Text' name = 'textbox_Hotel_game_id' require pattern='[0-9]{1,}' placeholder='Game id or course# here' title='The Games database id or its course number'>".
	...
	function createGamePage()
	{
			...
			"<h3>Course Number</h3>".
			"<input type = 'text' name = 'textbox_courseNumber' required pattern='[0-9]{5}' placeholder = '88529' title='5 digit number'>".
			
			"<h3>Course ID</h3>".
			"<input type = 'text' name = 'textbox_courseID' required placeholder = 'MAR2015'>".
			
			"<h3>Section</h3>".
			"<input type = 'text' name = 'textbox_section' required placeholder='U02'>".
			
			"<h3>Semester of Game</h3>".
			"<input type = 'text' name = 'textbox_semester' required placeholder='FALL 2016'>".
			
			"<h3>Course Meeting time</h3>".
			"<input type = 'text' name = 'textbox_schedule' required placeholder='mwf 12pm-1:30pm'>".

			"<h3>Period Start Date</h3>".
			"Year<br />".
			"<input type = 'text' name = 'textbox_PeriodStartYear' required pattern='[0-9]{4}' placeholder='2015' title='4 digit number'>".
			"<br />Month<br />".
			"<input type = 'text' name = 'textbox_PeriodStartMonth' required  pattern='[0-9]{2}' placeholder='08' title='2 digit number'>".
			"<br />Day<br />".
			"<input type = 'text' name = 'textbox_PeriodStartDay' required pattern='[0-9]{1,}' placeholder='20'>".
			
			"<h3>Period Length in days</h3>".
			"<input type = 'text' name = 'textbox_PeriodLength' required pattern='[0-9]{1,}' placeholder='13' title='length of period in days minus 1. ex. 2 weeks = 13 days'>".
			
			"<h3>Number of Periods</h3>".
			"<input type = 'text' name = 'textbox_NumberPeriods' required pattern='[0-9]{1,}' placeholder='15'>".
	...
	function createGame($semester,$courseID,$courseNumber,$section,$schedule,$periodYear,$periodMonth,$periodDay,$periodLength,$numberPeriods)
	{
		...
		{					   
			...
			$startDate = new DateTime($periodYear.'-'.$periodMonth.'-'.$periodDay);
		
			$endDate = new DateTime($startDate->format('Y-m-d '));
			$endDate->add(new DateInterval('P'.$periodLength.'D'));
			$endDate->setTime( 23,59,59 );
			
			for($int = 1;$int <= $numberPeriods;$int++)
			{
				if($mydatabase->addGame_period( $gameArray['id'], $startDate->format('Y-m-d H:i:s') ,  $endDate->format('Y-m-d H:i:s'), 1,$int) == 'fail')
					return "<b><h4>FAILED<br />Unable to make Game_period. Period: ".$int;
