...
class ReportController 
{
	...
	function CreatePageStudent($value)
	{	
		$result =  "<form action='' method='post'>".
						"<div class='bs-example'>".
							//"<div class='panel panel-default' style='padding: 0px 0px 150px 0px '>".
								"<div class='panel-body' align='center'>".
									"<h2>End of Period Comments</h2>".
								"</div>".
								"<div class='' align='center'>".
									"<div class='panel-footer clearfix' style='width: 1075px' >".
									$value.
	...
	function commentListStudent($email)
	{
		$mydatabase = new database();
		$student = $mydatabase->getStudent($email);
		$comments = $mydatabase->getCommentsByHotel($student['hotel']);
		...
		$result = ' <div class="table-responsive">'
					...
					.'</thead>';
		$int = 1;			
		foreach($comments as $key => $temp )
		{
			
			if(strlen($temp['comments'])>111)
				$temp['comments'] = substr(preg_replace( "/\n\s+/", "\n", rtrim(html_entity_decode(strip_tags( $temp["comments"] ))) ), 0,108)."...";
			if(strlen($temp['feedback'])>111)
				$temp['feedback'] = substr(preg_replace( "/\n\s+/", "\n", rtrim(html_entity_decode(strip_tags( $temp["feedback"] ))) ), 0,108)."...";
			...
			."<td>".$temp["comments"]."</td>"
			."<td>".$temp["feedback"]."</td>"
			."<td>"."<a href='http://marketsim-dev.cis.fiu.edu/views/reportingPage.php?button_viewfeedback=View+Feedback&commentId=".$temp['id']."' class='btn btn-primary'>View Feedback</a>"."</td>"
			."</tr>"
			."</tbody>";
		}
	...
	function feedbackPageStudent($id)
	{
		$mydatabase = new database();
		$comment = $mydatabase->getCommentById($id);
		$result = 	"<div align='center'>"
					...
					."<br />"
					."<h3>Feedback</h3>"
					."<textarea readonly rows='10' cols='60' name=textarea_feedback >".$comment['feedback']
					."</textarea>"
					."</div>"
					."<br />"
					."<br />"
					//."<input name='button_feedbackBack' type = 'submit' value = 'Back' class='btn btn-primary'/>";
					."<a href='http://marketsim-dev.cis.fiu.edu/views/reportingPage.php' class='btn btn-primary'>Back</a>";
