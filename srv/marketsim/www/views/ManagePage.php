...
else if(isset($_POST['button_addGame']) ) //add Game
{
	...
	$periodYear = $_POST['textbox_PeriodStartYear'];
	$periodMonth = $_POST['textbox_PeriodStartMonth'];
	$periodDay = $_POST['textbox_PeriodStartDay'];
	$periodLength = $_POST['textbox_PeriodLength'];
	$numberPeriods = $_POST['textbox_NumberPeriods'];
	
	if(	$periodYear < 1 || $periodMonth <1 || $periodDay < 1 || $periodLength < 1 || $numberPeriods < 1)
		$value = "<b>FAILED:</b> you entered in incorrect data. Values must be positive";
	else
		$value = $adminController->createGame($semester,$courseID,$courseNumber,$section,$schedule,$periodYear,$periodMonth,$periodDay,$periodLength,$numberPeriods);
}
